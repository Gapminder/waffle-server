...
module.exports = function (serviceLocator) {
  ...
  return {
    analyse: function (options, cb) {
      ...
      var tableName = options.tableName || 'Data';
      ...
      async.waterfall([
        ...
        function updateOrCreateIndicator(pipe, cb) {
          ...
          if (_.isArray(indicatorName)) {
            async.each(indicatorName, function (item) {
        ...
        function mergeIndicatorValues(pipe, cb) {
          ...
          async.parallel({
              // where ({worksheet:Data, row:{$gt:1}, column:1}) what ({dimension:country(type: Number)})
              countries: function distinctCountryValues(cb) {
                var rowQuery = {$gt: 1};
                var columnQuery = 1;
