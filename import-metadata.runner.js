...
const ACTION = `ACTION=${process.env.ACTION || 'metadata'}`;
...
const GOOGLE_CLIENT_ID = utils.config.GOOGLE_CLIENT_ID;
const GOOGLE_CLIENT_SECRET = utils.config.GOOGLE_CLIENT_SECRET;
const MONGODB_URL = utils.config.MONGODB_URL;
const NEO4J_URL = utils.config.NEO4J_URL;
const checkWithWhereIs = true;

const PARAMS = `${ACTION} ${NODE_ENV} ${LOG_LEVEL} ${LOG_TRANSPORTS} ${SESSION_SECRET} ${GOOGLE_CLIENT_ID} ${GOOGLE_CLIENT_SECRET} ${MONGODB_URL} ${NEO4J_URL}`;
