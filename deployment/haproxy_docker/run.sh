...
export VERSION=$(nodejs -p "require('./package.json').version")
export DOCKER_HOST="${HOSTNAME}"
export HOST="${STACK_NAME}-${MACHINE_SUFFIX}-${VERSION}-${RELEASE_DATE}-${DOCKER_HOST}"
...
echo -ne "export VERSION=\"${VERSION}\"\n" >> /etc/default/telegraf
