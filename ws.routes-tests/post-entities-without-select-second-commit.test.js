...
import ddfEntitiesWithoutSelectForPostRequest from './ws_ddf_test_fixtures_second_commit/entities-for-post-without-select.json';
...
test.cb('Check POST request: concepts without select when default dataset was set format=wsJson', t => {
  t.plan(1);
...
test.cb('Check POST request: concepts without select when default dataset was set format=json', t => {
  t.plan(1);
...
test.cb('Check POST request: concepts without select when default dataset was set format=ddfJson', t => {
  t.plan(1);
