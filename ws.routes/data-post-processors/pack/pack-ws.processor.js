...
function mapSchemaToWsJson(data) {
  ...
  return { name: data.datasetName, version: data.datasetVersion, headers, rows: sortRows(rows, data.query, headers)};
}
...
function mapConceptToWsJson(data) {
  ...
  return { name: data.datasetName, version: data.datasetVersion, headers: select, rows: sortRows(rows, data.query, data.headers) };
}
