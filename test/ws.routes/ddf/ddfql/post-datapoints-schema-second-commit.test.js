...
const ddfDatapointsSchemaForPostRequest = require('./ws_ddf_test_fixtures_second_commit/datapoints-schema-wsjson.json');
...
it('Check POST request: datapoints.schema when default dataset was set', done => {
  ...
  api.post(`/api/ddf/ql?format=wsJson`)
    ...
    .end((error, res) => {
      expect(res.body).to.deep.equal(ddfDatapointsSchemaForPostRequest);
